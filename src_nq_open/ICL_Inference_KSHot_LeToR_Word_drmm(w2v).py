...
from nq_utils import tokenizer, model, device, compute_f1, sanitize_query, extract_features, extract_drmm_w2v_features, build_w2v_model

# --------------------------
# Config
# --------------------------
TEST_IN = "/scratch/2980356s/data/SST2/output/nq_open_top15_bm25_llm_pairs/nq_open_top15_bm25_llm_pairs_test.csv.gz"
FEATURED_TEST_IN = "/scratch/2980356s/data/SST2/output/nq_open_top15_bm25_llm_pairs/word12_drmmw2v_features/nq_open_top15_bm25_llm_pairs_test_word12_drmmw2v.csv.gz"
IDF_CACHE_PKL = "/scratch/2980356s/data/SST2/output/nq_open_top15_bm25_llm_pairs/word12_features/idf_from_terrier.pkl"
MODEL_FILE_DESC = "lambdamart_nq_word12_drmmw2v_desc.txt"

OUT_DIR = "/scratch/2980356s/data/SST2/output/nq_open_top15_bm25_llm_pairs/icl_eval_word12_drmmw2v"
...
def ensure_features(df, idf_dict, w2v_model):
    ...
    feats = df.apply(
        lambda r: extract_features(str(r["query"]), str(r["text"]), idf_dict) +
                  extract_drmm_w2v_features(str(r["query"]), str(r["text"]), w2v_model),
...
def run_inference(order_label, model_ltr, df, idf_dict, w2v_model):
    ...
    out_csv = os.path.join(OUT_DIR, f"icl_infer_word12_drmmw2v_{order_label}.csv.gz")
...
def main():
    ...
    # train W2V on test set texts (same as precompute script)
