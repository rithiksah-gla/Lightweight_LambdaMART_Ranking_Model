...
import torch
from nq_utils import tokenizer, model, device, compute_f1, sanitize_query, extract_features

TEST_IN = "/scratch/2980356s/data/SST2/output/nq_open_top15_bm25_llm_pairs/nq_open_top15_bm25_llm_pairs_test.csv.gz"
FEATURED_TEST_IN = "/scratch/2980356s/data/SST2/output/nq_open_top15_bm25_llm_pairs/word12_features/nq_open_top15_bm25_llm_pairs_test_word12.csv.gz"
IDF_CACHE_PKL = "/scratch/2980356s/data/SST2/output/nq_open_top15_bm25_llm_pairs/word12_features/idf_from_terrier.pkl"
MODEL_FILE_DESC = "lambdamart_nq_word12_desc.txt"

OUT_DIR = "/scratch/2980356s/data/SST2/output/nq_open_top15_bm25_llm_pairs/icl_eval_word12"
...
def ensure_features(df, idf_dict):
    if all(c in df.columns for c in FEATURE_NAMES):
        return df
    feats = df.apply(lambda r: extract_features(str(r["query"]), str(r["text"]), idf_dict),
                     axis=1, result_type="expand")
    feats.columns = FEATURE_NAMES
    return pd.concat([df, feats], axis=1)

def run_inference(order_label, model_ltr, df, idf_dict):
    results = []
    for qid, group in tqdm(df.groupby("qid"), desc=f"ICL Inference ({order_label.upper()})"):
        ...
        # feature scoring
        g = ensure_features(group, idf_dict).copy()
    ...
    out_csv = os.path.join(OUT_DIR, f"icl_infer_word12_{order_label}.csv.gz")
    out_df.to_csv(out_csv, index=False, compression="gzip")

    # micro P/R/F1 (mean-based)
...
def main():
    ...
    # run both DESC and ASC
    run_inference("desc", model_ltr, df, idf_dict)
    run_inference("asc", model_ltr, df, idf_dict)
