# lambdamart_train_nq_word_drmmw2v_phi.py
import os
import pickle
import pandas as pd
import numpy as np
...
from nq_utils import (
    extract_features, build_w2v_model,
    extract_drmm_w2v_features, extract_phi_w2v_features
)
...
# ---- Train Word2Vec ----
print("Training Word2Vec (train corpus)...")
w2v_corpus = train_df["text"].fillna("").tolist()
...
FEATURE_NAMES = WORD12 + DRMM20 + PHI15   # 47 features
...
def add_features(df):
    ...
    for _, r in df.iterrows():
        ...
        f_drmm = extract_drmm_w2v_features(q, d, w2v_model)
        f_phi  = extract_phi_w2v_features(q, d, w2v_model)
        feats.append(f_word + f_drmm + f_phi)
...
train_and_save_model(train_desc, val_desc, "lambdamart_nq_word_drmmw2v_phi_desc.txt")
train_and_save_model(train_asc,  val_asc,  "lambdamart_nq_word_drmmw2v_phi_asc.txt")
print("Done.")
