...
from nq_utils import (
    ...
    extract_features, build_lda_model, extract_lda_features, extract_drmm_features
)

# -------- Config --------
TEST_IN = "/scratch/2980356s/data/SST2/output/nq_open_top15_bm25_llm_pairs/nq_open_top15_bm25_llm_pairs_test.csv.gz"
FEATURED_TEST_IN = "/scratch/2980356s/data/SST2/output/nq_open_top15_bm25_llm_pairs/word_lda_drmm_features/nq_open_top15_bm25_llm_pairs_test_word_lda_drmm.csv.gz"
...
OUT_DIR = "/scratch/2980356s/data/SST2/output/nq_open_top15_bm25_llm_pairs/icl_eval_word_lda_drmm"
...
def ensure_features(df, idf_dict, lda_model, lda_dict):
    ...
    feats = df.apply(
        lambda r: extract_features(str(r["query"]), str(r["text"]), idf_dict) +
                  extract_lda_features(str(r["query"]), str(r["text"]), lda_model, lda_dict, num_topics=50) +
                  extract_drmm_features(str(r["query"]), str(r["text"]), lda_model, lda_dict, num_topics=50, num_bins=20),
...
def run_inference(order_label, model_ltr, df, idf_dict, lda_model, lda_dict):
    ...
    out_csv = os.path.join(OUT_DIR, f"icl_infer_word_lda_drmm_{order_label}.csv.gz")
...
def main():
    ...
    needed_cols = {"qid","query","text","ground_truth"}
    ...
    # LDA on test corpus (texts + queries); no caching per your preference
    ...
    # both orders (DESC model for both)
