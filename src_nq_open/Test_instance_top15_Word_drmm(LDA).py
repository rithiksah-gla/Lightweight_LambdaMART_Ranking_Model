# build_word12_lda_drmm_features_for_nq_test.py
import os
import pickle
import pandas as pd
...
OUT_DIR = os.path.join(os.path.dirname(TEST_IN), "word12_lda_drmm_features")
...
OUT_CSV = os.path.join(
    OUT_DIR,
    os.path.basename(TEST_IN).replace(".csv.gz", "_word12_lda_drmm.csv.gz")
)
...
def main():
    ...
    print("Extracting Word(12) + LDA-DRMM(20) features ...")
    feats_list = []
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        feats_list.append(f_word + f_drmm)
