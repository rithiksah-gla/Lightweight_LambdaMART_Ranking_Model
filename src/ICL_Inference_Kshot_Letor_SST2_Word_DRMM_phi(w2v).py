...
# from utils import (
#     extract_features,
#     extract_drmm_w2v_features,
#     extract_phi_w2v_features,
#     get_idf_dict,
#     build_w2v_model,
# )

# Precompute file with 47 features (12 + 20 DRMM-w2v + 15 phi-w2v)
df = pd.read_csv("/kaggle/working/test_query_top50_word_DRMM_phi_w2v_features.csv")

# Build IDF + W2V from the same text pool (queries + candidates)
...
def predict_label_llm(prompt: str) -> int:
...
def run_kshot_letor_inference(order="desc"):
    # Use DESC model for BOTH asc and desc runs (as requested)
    model_path = "/kaggle/working/lambdamart_with_word_DRMM_phi_w2v_features_desc.txt"
    ...
    for qid, group in tqdm(grouped, total=len(grouped), desc=f"K-shot LeToR ({order})"):
        ...
        q_label = int(group.iloc[0]["query_label"])
        queries.append(query)

        # Recompute the 47 features used by the model: 12 + 20 DRMM-w2v + 15 phi-w2v
        feats = []
        for _, row in group.iterrows():
            cand = row["candidate"]
            f = (
                extract_features(query, cand, idf_dict)
                + extract_drmm_w2v_features(query, cand, w2v_model)
                + extract_phi_w2v_features(query, cand, w2v_model)
            )
            feats.append(f)
        ...
        # top-10 by DESC score; for "asc" we just reverse those top-10 for the prompt order
    ...
    pd.DataFrame(
        {"query": queries, "query_label": y_true, "predicted_label": y_pred}
    ).to_csv(f"kshot_letor_predictions_{order}_47_w2v.csv", index=False)

    print(f"\nK-shot LeToR ICL Evaluation ({order.upper()}):")
    print(classification_report(y_true, y_pred, target_names=["negative", "positive"], digits=4))

# Run both orders (both use DESC model under the hood)
