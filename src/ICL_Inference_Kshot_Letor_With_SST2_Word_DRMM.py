# ICL LeToR inference for 32 features (12 handcrafted + 20 DRMM)
...
#from utils import extract_features, extract_drmm_features, get_idf_dict, build_lda_model

# Load retrieved top-50 SBERT candidates (32-feature precompute)
df = pd.read_csv("/kaggle/input/word-and-drmm-using-lda-vec/test_query_top50_word_DRMM_features.csv")

# Build IDF dictionary and LDA model (DRMM needs LDA for per-token topic vectors)
texts = df["query"].tolist() + df["candidate"].tolist()
idf_dict = get_idf_dict(texts)
lda_model, lda_dict = build_lda_model(texts)

# Load LLaMA model and tokenizer
hf_token = "hf***"  # put your token
...
def run_kshot_letor_inference(order="desc"):
    # Use ASC model for ASC; DESC model for DESC
    model_path = "/kaggle/input/word-and-drmm-using-lda-vec/lambdamart_with_word_DRMM_features_desc.txt"
    ...
    for test_query_id, group in tqdm(grouped, total=len(grouped), desc=f"K-shot LeToR ({order})"):
        ...
        # Compute 32 features per candidate: 12 handcrafted + 20 DRMM
        features = []
        ...
        # For ASC prompts, reverse the order of the top-10; for DESC keep as is
